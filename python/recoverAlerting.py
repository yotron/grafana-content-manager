...
class recoverProcesses:
    def __init__(self, instSetting):
        ...
        self.dataSourceFolder = funcs.getDataSourceFolder(instSetting["name"])
    ...
    def recoverAlertRules(self):
        ...
        for alertFile in alertList:
            ...
            manAlertJson = self.changeUids(manAlertJson, self.mappings)
    ...
    def changeUids(self, dict, uidMappings):
        for dataNr, data in enumerate(dict["data"]):
            if "datasourceUid" in data:
                sourceDsUid = data["datasourceUid"]
                if sourceDsUid in uidMappings.keys():
                    dict["data"][dataNr]["datasourceUid"] = uidMappings[sourceDsUid]
            if "model" in data and "datasource" in data["model"] and "uid" in data["model"]["datasource"]:
                sourceDsUid = data["model"]["datasource"]["uid"]
                if sourceDsUid in uidMappings.keys():
                    dict["data"][dataNr]["model"]["datasource"]["uid"] = uidMappings[sourceDsUid]
        return dict